...
power.z.test <- function(n1=NULL, sigma1=0.5, 
  ...
  if(!is.null(n2)){
    ...
    X_bar1 = Z*denominador1 + mu.Ha
    X_bar1_neg = -Z*denominador1 + mu.Ha
    X_bar2 = Z*denominador2 + mu.True
    X_bar = X_bar1 - X_bar2
    
    numerador = X_bar - (mu.Ha - mu.True)
    denominador = sqrt(denominador2^2 + denominador1^2)
    Z = numerador/denominador
    Power = 1 - pnorm(Z)
    
    # Se calculan los límites gráficos de power, 
    min_lim = min(rnorm(1000, mean=mu.Ha, sd=denominador1)) - 
      round(min(rnorm(1000, mean=mu.Ha, sd=denominador1)))%%10
    max_lim = max(rnorm(1000, mean=mu.True, sd=denominador2)) +
      round(max(rnorm(1000, mean=mu.True, sd=denominador2)))%%10
      
    # Ploteo del power del test de hipotesis.
    # En rojo queda la distribución del test de hipótesis, mientras que en azul la distribución 2
    # Además, el área llenada en rojo es la visualización del power.
    plot <- ggplot(data.frame(x = c(min_lim, max_lim)), aes(x)) + 
      stat_function(fun = dnorm, args = list(mean = mu.Ha, sd = denominador1), 
                    col='red') +
      stat_function(fun = dnorm, args = list(mean = mu.True, sd = denominador2), 
                    col='blue') +
      stat_function(fun = dnorm, args = list(mean = mu.True, sd = denominador2), 
                    xlim = c(X_bar1, max_lim), geom = "area", fill='red') + 
      geom_vline(xintercept = X_bar1, linetype="dotted", size=1) +
      geom_vline(xintercept = X_bar1_neg, linetype="dotted", size=1) +
      annotate(x=X_bar1, y=+Inf,label="alpha/2", vjust=2, geom="label") +
      annotate(x=X_bar1_neg, y=+Inf,label="-alpha/2", vjust=2, geom="label") +
      theme_minimal() +
      ggtitle("H0 vs Ha") +
      xlab(expression(bar(X))) + ylab("Density")
    }
...
# REVISAR EL VALOR DEL POWER PORQUE PARECE QUE ESTÁ MALO.
p <- power.z.test(n1=50, sigma1=20, n2=50, sigma2=20, mu.Ha=100, mu.True = 110, alfa=0.05)
p
```
